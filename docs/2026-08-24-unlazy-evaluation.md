...
- **D. Adopt the format, not the code.** Accepted, shipped separately as `skills/maestro/references/evidence-ledger.md` plus CLASSIFY, Step 4 and Step 8.0 wiring. The two properties unlazy genuinely has that maestro's prose gates lack are pre-commitment (the oracle is fixed before implementation, not chosen afterwards to fit the result) and a durable unmet state that survives compaction. Both are format properties, not script properties. A maestro-owned ledger captures them with no third-party code, no Stop hook, no approval store, and with unlazy's ABANDON discipline chained into the final report's unmet-gate enumeration. Attribution to unlazy's ledger format is carried in the reference file, as MIT asks.
...
| Claim | Source |
|---|---|
| Repo metadata (creation, push, stars, commits, contributors) | GitHub API, 2026-08-24 |
| Engineering-quality assertions, runtime and licence metadata | Source-level read of the clone at HEAD `754d9a6` |
...
Deliberately not changed: `skills/maestro/SKILL.md`, `skills/maestro/references/ecosystem.md`, `install.sh`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.github/workflows/track-upstream.yml`. No version ships with this record. The evidence-ledger follow-up and the repository fixes shipping in the same session land as their own PRs with their own version bumps.
