...
13. **Reports through a Progress Protocol** — while a flow is active, every response opens with its position ("Step 5 of 10, blocked on mockup approval"), closes on exactly one open action, puts fresh command output above the justification prose, and never estimates wall-clock time (duration is an unevidenced claim). Adapted from cognitive-accessibility formatting practice; it shapes reporting only and never truncates a checklist or drops a justification
